...
[A-1] Logit shaping 从 generate() 解耦为 MemLLM.shape_step_logits() 公共方法
      → 修复 4.15: runner 的 stepwise decode 也能享受 F-2 hard mask
      → 修复 4.12: runner 路径因此不再 early collapse

[A-2] MemLLM.prepare_decode_context() 公共方法
      → 一次性产出 (prefix_cond, prefix_uncond, content_bias, suppression_bias, vocab_bias)

[A-3] Trainer.recon() 公共方法 (真实 API, 非 shim)
      → 修复 4.14

[A-4] DecodeState 数据类 → runner 和 generate() 共享同一状态语义

保留的 v3.32 机制: F-1 contrastive suppression / F-3 adaptive scaling /
                   F-4 retrieval thresholds / F-5 bypass gate open /
                   F-6 cfg=3.5 / S-1/S-2/S-3/S-4
"""
...
@dataclass
class DecodeState:
    """Shared state semantics between MemLLM.generate() and external runners."""
    ...
    def update(self, nxt_id: int, step: int, content_classifier,
               bpe_echo_window: int, cyclic_content_window: int):
...
@dataclass
class DecodeContext:
    """Public context produced by prepare_decode_context(). Immutable handle
    used by external runners + internal generate()."""
...
class MemLLM(nn.Module):
    ...
    # ─────────────────────────────────────────────────────────────
    # [A-2] Public API: prepare_decode_context
    # ─────────────────────────────────────────────────────────────
    def prepare_decode_context(self, ids: torch.Tensor, mask: torch.Tensor,
                               update_stats: bool = True) -> DecodeContext:
        """Build full decode context for a given prompt (ids/mask)."""
        with torch.no_grad():
            o = self.fwd(ids, mask)
        ...
        return DecodeContext(
            ...
            fiber_summary=fiber_summary, diag=diag,
            content_bias=content_bias, suppression_bias=suppression_bias,
            vocab_bias=vocab_bias)

    # ─────────────────────────────────────────────────────────────
    # [A-1] Public API: shape_step_logits
    # ─────────────────────────────────────────────────────────────
    def shape_step_logits(self,
        ...
        """Apply the complete v3.33 logit-shaping pipeline for one decode step."""
        c = self.c
        dev = logits_cond.device
        cc = self.content_classifier
        HARD_MASK = -1e9
        ...
        if c.use_cyclic_content_hard_mask and cc is not None:
            window = c.cyclic_content_window
            max_cnt = c.cyclic_content_max_count
            window_counts: Dict[int, int] = {}
            cutoff_step = step - window
        ...
        if c.use_newline_hard_gate and cc is not None:
            ...
            hard_gate_active = (
                step < c.newline_hard_gate_min_step
...
class Trainer:
    ...
    # ─────────────────────────────────────────────────────────────
    # [A-3] Public API: recon
    # ─────────────────────────────────────────────────────────────
    def recon(self, text: str) -> Dict[str, torch.Tensor]:
