...
嵌入级方案B · v3.41
═══════════════════════════════════════════════════════════════════════════
针对 v3.40 10 条 FAIL 的结构性根因修复:

[G-1] 4.17 save_load 非位精确 → MemoryContextEncoder 去 mean-center;
      encode 路径全 fp32;save 前 .detach().contiguous().cpu()。

[G-2] 4.22/4.10 guidance=False 导致 fwd 路径 FS 永不激活
      → _get_prefix(return_extra=True) 分支按 _check_guidance_active(diag)
        设置 guidance_active 并附加 content/suppression biases 到 prefix。
        shape_step_logits 仍复加(独立路径),fwd 的 dampen 保证总量不超。

[G-3] 4.23 双 LN 滤掉 residual dominant 方向 + 4.12 attractor basin
      → rare_keyword_wte_residual 去掉 √d_LLM 缩放,保持 WTE 原生幅值;
        在 bridge.inject 的 **post-aligner** 位置做 α=0.5 的加法 blend。

[G-4] 4.24 encoder 输入 hidden_mean 天然 inter 高
      → MemoryContextEncoder.encode_from_tokens(content_token_ids, wte)
        使用 WTE 严格 content-starter centroid 作为编码输入;

[G-5] 4.25 扩容 slot 在 eval 下为噪声源
      → ContentSemanticTailHead 的 s>=2 slot 共享权重到 slot_heads[1];

[G-6] 4.15 content_bias × dampen 不足以跨 stopword gap
      → fwd_function_suppression 的 scale 从 fwd_path_bias_dampen 中解耦。

[G-7] 附带:Trainer 新增 context_separation_loss 对 encoder 提供定向信号。
"""
...
class Cfg:
    ...
    # [G-6] fwd-path function suppression: DECOUPLED from fwd_path_bias_dampen
    ...
    # [G-3] WTE residual: post-aligner blend at native WTE scale
    use_wte_residual_tail: bool = True
    wte_residual_alpha: float = 0.5
    ...
    # [G-4] Context encoder uses WTE centroid of strict-content-starter tokens
    context_encoder_source: str = "wte_strict_starter"
    context_encoder_fp32: bool = True
    # [G-7] Optional training loss (warmup-gated)
    ...
    def __post_init__(self):
        ...
        assert 0.0 <= self.wte_residual_alpha <= 2.0
...
class ContentSemanticTailHead(nn.Module):
    """[G-5] Tied extra slots: only two distinct heads (slot 0 + slot 1+)."""
    ...
    def forward(self, fiber_summary):
        """Return raw tail output (no residual); residual applied post-aligner."""
...
class MemoryContextEncoder(nn.Module):
    """
    [G-4] Takes WTE centroid (d_LLM) of strict-content-starter tokens.
    [G-1] No mean-center. Entire forward runs fp32. Output .detach().contiguous().
    """
...
class EmbBridge(nn.Module):
    ...
    def inject(self, fibers, mem_mask=None, fiber_summary=None,
        ...
        if (self._effective_tail_slots > 0 and fiber_summary is not None):
            # [G-3] residual applied POST-aligner
...
class MemLLM(nn.Module):
    ...
    def _compute_rare_keyword_wte_residual(self, diag):
        """[G-3] Return raw WTE centroids per slot (no √d scaling)."""
    ...
    def fwd(self, ids, mask, prefix=None):
        ...
        if self.c.use_fwd_path_content_bias and (content_bias is not None
            ...
            if suppression_bias is not None and self.c.use_memory_guided_suppression:
                ...
                mod_last = True
            # [G-6] function suppression: DECOUPLED from content bias dampen
            if need_fs:
    ...
    def _get_prefix(self, hs, mask=None, pl=0, update_stats=True, return_extra=False, ids=None):
        ...
        # [G-2] Uniform guidance handling for BOTH return_extra branches.
    ...
    def save_memory(self, path):
        """[G-1] .detach().contiguous().cpu() for byte-identical reload."""
...
class Trainer:
    ...
    def context_separation_loss(self, texts):
        """[G-7] Pushes context descriptors of disjoint-starter texts apart."""
        ...
        for ss in per_text_strict_ids:
            ...
            centroid = wte.index_select(0, idx).float().mean(0)
            h = self.m.memory_context_encoder.net(centroid)
