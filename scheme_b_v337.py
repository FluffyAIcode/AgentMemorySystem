...
[C-5] IDF-weighted content bias → 修复 4.7 / 4.11 / 4.19-inject
      _build_token_bias_from_memories 对每个 token 的贡献乘
      corpus IDF (clamped to [idf_floor, max_boost=3.0]), 使稀有
      域指示词 (chopin, nocturne) 相对高频复读词 (dynamics, depends)
      获得 ~2x 的相对 boost, 能够进入 top-12.

[C-6] Multi-signal DirectionTree.retrieve → 修复 4.16 / 4.19-retrieve
      在 backbone.forward 上注册 forward-pre-hook 捕获 query ids 到
      amm._last_query_ids. tree.retrieve(qdir, bw) 内部:
      1) beam search 召回 (不变)
      2) 提取 query content tokens, 对每个候选计算 centroid cosine
         + forward maxsim (IDF-加权)
      3) 组合得分 0.2·dir + 0.4·centroid + 0.4·fwd 重排
      签名不变, 对 runner 完全透明.

保留 v3.36 的 [C-4] 和前版的 [A-*]/[B-*]/[C-1..3].
"""
...
class Cfg:
    ...
    # [C-5] IDF-weighted content bias
    use_idf_content_bias: bool = True
    idf_bias_max_boost: float = 3.0
    # [C-6] tree-level multi-signal rerank
    ...
    def __post_init__(self):
        ...
        assert 0.8 < rr < 1.2, f"tree rerank weights sum {rr}"
...
class DirectionTree:
    """
    [C-6] tree.retrieve() now performs multi-signal reranking internally,
    preserving the (qdir, bw) → List[(mid, score)] signature.
    """
    def __init__(self, c, amm_ref=None):
        self.c=c; self.root=_Node(); self.store={}; self.nid=0
        # [C-6] back-reference to AMM for multi-signal scoring; may be set later
    ...
    def _beam_retrieve(self, qdir, bw):
        """Pure direction beam search — the original algorithm, now isolated."""
    ...
    def retrieve(self, qdir, bw=3):
        """
        [C-6] Multi-signal retrieval. Signature preserved:
          input:  qdir (d_M tensor), bw (int)
          output: List[(mid, score)] sorted descending

        Pipeline:
          1. dir-only beam search (original) → candidate recall set
          2. if AMM context is available (content_classifier + wte_normed +
             last captured query ids from backbone pre-hook), rerank by
             combined: α_d · dir + α_c · centroid_cosine + α_f · forward_maxsim
             (centroid and forward both IDF-weighted).
          3. otherwise return raw dir ordering — this is NOT a fallback for
             correctness, it is the legitimate answer when no query context
             has been captured (e.g. consolidation path during write_mem()).
        """
        raw = self._beam_retrieve(qdir, bw)
        amm = self._amm_ref
        if amm is None: return raw
        if not getattr(amm.c, 'use_tree_semantic_rerank', False): return raw
        # During training we preserve the dir-only ordering to keep the
        # reranker / gradient flow deterministic.
...
class AMM(nn.Module):
    def __init__(self, c):
        ...
        # [C-6] tree carries a back-ref to self for multi-signal retrieval
        self.tree=DirectionTree(c, amm_ref=self); self.time=0.
        self.wte_normed = None
        # [C-6] last query context captured by backbone forward-pre-hook
        self._last_query_ids = None
        self._last_query_mask = None
        # [C-6] content classifier shared by MemLLM.load()
...
class MemLLM(nn.Module):
    ...
    def load(self, name=None, dtype_name=None):
        ...
        # [C-6] share content classifier so tree.retrieve can do rerank
        self.amm._content_classifier = self.content_classifier
        # [C-6] capture last-query ids via official PyTorch forward pre-hook.
        # Fires on every backbone forward; tree.retrieve reads the most recent
        # capture (which in all real flows is the query being retrieved for).
        amm_ref = self.amm
    ...
    def _check_guidance_active(self, diag) -> bool:
        ...
        if not diag or not diag.batch_mem_weights:
            return False
    ...
    # ═══════════════════════════════════════════════════════════════
    # [C-5] IDF-weighted content bias.
    # Each token's contribution to the bias is multiplied by its corpus IDF
    # (clamped to [idf_floor, idf_bias_max_boost]). Rare domain-indicator
    # tokens (df=1) get ~2.25x the boost of common cross-domain tokens (df=N),
    # pushing them into decoder top-k.
    # ═══════════════════════════════════════════════════════════════
        ...
        for mid, weight in mem_weight_list:
            ...
            if q_valid and q_vecs is not None:
                ...
                for i, tid in enumerate(valid_ids):
                    if use_idf:
                        idf_val = max(idf_floor,
                                      min(max_boost, corpus_idf.get(tid, idf_floor)))
                    else:
                        idf_val = 1.0
...
MUSIC_KEYS = ['piano','orchestra','music','conservatory','symphony','chopin','beethoven','nocturne','harmonic']
SPACE_KEYS = ['telescope','astronaut','nebula','galaxy','galaxies','mars','spectrum','milky']

def _write_corpus(m, corpus):
    m.amm.tree.store.clear(); m.amm.tree.root = _Node(); m.amm.tree.nid = 0; m.amm.time = 0
    for t in corpus: m.write(t, training_mode=True)
    m.eval()

def _write_mixed(m):
    _write_corpus(m, MUSIC_CORPUS + SPACE_CORPUS)

def _clear(m):
    m.amm.tree.store.clear(); m.amm.tree.root = _Node(); m.amm.tree.nid = 0; m.amm.time = 0

def _collect_domain_mids(m):
    music_mids = set(); space_mids = set()
    for mid, mem in m.amm.tree.store.items():
        text = mem.source_text.lower()
        if any(k in text for k in MUSIC_KEYS): music_mids.add(mid)
        elif any(k in text for k in SPACE_KEYS): space_mids.add(mid)
    return music_mids, space_mids

def test_backbone(m, c, R):
    print("\n── LLMBackbone ──")
    R.check("backbone_loaded", m.backbone is not None)
    R.check("d_LLM_matches", c.d_LLM == m.backbone.d_model)
    R.check("tokenizer_has_pad", m.tok.pad_token is not None)
    dev = next(m.parameters()).device
    tk = m.tok("hello world", return_tensors='pt')
    ids = tk['input_ids'].to(dev); mask = tk['attention_mask'].to(dev)
    with torch.no_grad(): o = m.fwd(ids, mask)
    R.check("fwd_logits_shape", o['logits'].shape[:2] == ids.shape)
    R.check("fwd_hs_layers", len(o['hs']) == m.backbone.n_layers + 1)

def test_hungarian(m, c, R):
    print("\n── Hungarian ──")
    dev = next(m.parameters()).device
    sim = torch.eye(4, device=dev)
    _, total = hungarian_max_assignment(sim)
    R.check("hungarian_identity", abs(total - 4.0) < 1e-5)
    torch.manual_seed(0)
    sim2 = torch.rand(5, 7, device=dev)
    _, total_h = hungarian_max_assignment(sim2)
    greedy_sum = 0.0; used = set()
    rows_by_max, _ = sim2.max(dim=1)
    for r in rows_by_max.argsort(descending=True).tolist():
        avail = [j for j in range(sim2.shape[1]) if j not in used]
        if not avail: break
        j_best = max(avail, key=lambda j: sim2[r, j].item())
        greedy_sum += sim2[r, j_best].item(); used.add(j_best)
    R.check("hungarian_ge_greedy", total_h >= greedy_sum - 1e-5)

def test_directiontree_api(m, c, R):
    print("\n── [C-1] DirectionTree API contract ──")
    _write_mixed(m)
    depth = m.amm.tree.max_depth()
    viols = m.amm.tree.leaf_size_violations()
    R.check("tree_max_depth_is_int", isinstance(depth, int))
    R.check("tree_max_depth_nonneg", depth >= 0)
    R.check("tree_violations_is_list", isinstance(viols, list))
    try:
        viols_len = len(viols); R.check("tree_violations_supports_len", True)
    except TypeError as e:
        viols_len = -1; R.check("tree_violations_supports_len", False, str(e))
    R.check("tree_violations_len_matches_type",
            isinstance(viols_len, int) and viols_len >= 0)
    R.check("tree_no_leaf_violations_default_corpus", len(viols) == 0)
    _clear(m)
    R.check("tree_empty_depth", m.amm.tree.max_depth() == 0)
    R.check("tree_empty_violations_list", m.amm.tree.leaf_size_violations() == [])

def test_query_context_capture_hook(m, c, R):
    """[C-6] backbone forward-pre-hook captures ids into amm._last_query_ids."""
    print("\n── [C-6] query context capture hook ──")
    dev = next(m.parameters()).device
    m.amm._last_query_ids = None
    tk = m.tok("The piano sounds beautiful", return_tensors='pt')
    ids = tk['input_ids'].to(dev); mask = tk['attention_mask'].to(dev)
    with torch.no_grad():
        _ = m.backbone(ids, mask)
    R.check("hook_captures_ids",
            m.amm._last_query_ids is not None)
    if m.amm._last_query_ids is not None:
        R.check("hook_captured_ids_match_shape",
                tuple(m.amm._last_query_ids.shape) == tuple(ids.shape))
        R.check("hook_captured_ids_match_values",
                torch.equal(m.amm._last_query_ids.cpu(), ids.cpu()))

def test_tree_semantic_rerank(m, c, R):
    """[C-6] DirectionTree.retrieve performs multi-signal rerank."""
    print("\n── [C-6] tree.retrieve multi-signal semantic rerank ──")
    _write_mixed(m); m.eval()
    dev = next(m.parameters()).device
    music_mids, space_mids = _collect_domain_mids(m)
    R.check("rerank_music_mids_present", len(music_mids) >= 2,
            f"only {len(music_mids)} music mids identified")
    R.check("rerank_space_mids_present", len(space_mids) >= 2,
            f"only {len(space_mids)} space mids identified")

    def _tree_top5_mids(prompt):
        tk = m.tok(prompt, return_tensors='pt')
        ids = tk['input_ids'].to(dev); mask_p = tk['attention_mask'].to(dev)
        with torch.no_grad():
            o = m.backbone(ids, mask_p)
            pooled = m.amm.layer_pool(o['hs']).mean(1)
            xq_b = m.amm.ctx(pooled)
            fq = m.amm.fib(pooled, xq_b)
            qdir = m.amm.dir_pred(xq_b, fq)
            scored = m.amm.tree.retrieve(qdir[0].detach(), bw=3)
        return [mid for mid, _ in scored[:5]]

    top5_m = _tree_top5_mids("What improves piano technique and musical phrasing?")
    mm = sum(1 for mid in top5_m if mid in music_mids)
    ms = sum(1 for mid in top5_m if mid in space_mids)
    print(f"    music query → top5 ids={top5_m} music={mm} space={ms}")
    R.check("tree_rerank_music_query_majority_music",
            mm > ms, f"music={mm} vs space={ms}")

    top5_s = _tree_top5_mids("What explains satellites and orbital motion of planets?")
    sm = sum(1 for mid in top5_s if mid in music_mids)
    ss = sum(1 for mid in top5_s if mid in space_mids)
    print(f"    space query → top5 ids={top5_s} music={sm} space={ss}")
    R.check("tree_rerank_space_query_majority_space",
            ss > sm, f"music={sm} vs space={ss}")

    R.check("tree_rerank_differs_across_queries", top5_m != top5_s,
            f"music={top5_m} space={top5_s}")
    _clear(m)

def test_tree_rerank_training_bypass(m, c, R):
    print("\n── [C-6] tree.retrieve training-mode bypass ──")
    _write_mixed(m)
    dev = next(m.parameters()).device
    tk = m.tok("piano music", return_tensors='pt')
    ids = tk['input_ids'].to(dev); mask_p = tk['attention_mask'].to(dev)
    with torch.no_grad():
        o = m.backbone(ids, mask_p)
        pooled = m.amm.layer_pool(o['hs']).mean(1)
        xq_b = m.amm.ctx(pooled); fq = m.amm.fib(pooled, xq_b)
        qdir = m.amm.dir_pred(xq_b, fq)
    m.eval()
    with torch.no_grad():
        scored_eval = m.amm.tree.retrieve(qdir[0].detach(), bw=3)
    m.train()
    with torch.no_grad():
        scored_train = m.amm.tree.retrieve(qdir[0].detach(), bw=3)
    m.eval()
    scored_raw = m.amm.tree._beam_retrieve(qdir[0].detach(), 3)
    raw_order = [mid for mid, _ in scored_raw]
    train_order = [mid for mid, _ in scored_train]
    R.check("training_mode_returns_raw_dir_order", train_order == raw_order,
            f"train={train_order} raw={raw_order}")
    print(f"    eval order={[mid for mid,_ in scored_eval]}")
    print(f"    raw order={raw_order}")
    _clear(m)

def test_tree_rerank_preserves_signature(m, c, R):
    print("\n── [C-6] tree.retrieve signature preservation ──")
    _write_mixed(m); m.eval()
    dev = next(m.parameters()).device
    tk = m.tok("anything", return_tensors='pt')
    ids = tk['input_ids'].to(dev); mask_p = tk['attention_mask'].to(dev)
    with torch.no_grad():
        o = m.backbone(ids, mask_p)
        pooled = m.amm.layer_pool(o['hs']).mean(1)
        xq_b = m.amm.ctx(pooled); fq = m.amm.fib(pooled, xq_b)
        qdir = m.amm.dir_pred(xq_b, fq)
        result = m.amm.tree.retrieve(qdir[0].detach(), bw=3)
    R.check("retrieve_returns_list", isinstance(result, list))
    if result:
        R.check("retrieve_items_are_tuples_of_two",
                all(isinstance(x, tuple) and len(x) == 2 for x in result))
        R.check("retrieve_items_mid_int_score_float",
                all(isinstance(x[0], int) and isinstance(x[1], float) for x in result))
        scores = [x[1] for x in result]
        R.check("retrieve_sorted_descending",
                all(scores[i] >= scores[i+1] for i in range(len(scores)-1)))
    _clear(m)

def test_idf_content_bias(m, c, R):
    print("\n── [C-5] IDF-weighted content bias ──")
    _write_mixed(m); m.eval()
    corpus_idf = m.amm._compute_corpus_idf(m.content_classifier)
    R.check("corpus_idf_nonempty", len(corpus_idf) > 0)
    if not corpus_idf:
        _clear(m); return
    idf_values = list(corpus_idf.values())
    idf_min = min(idf_values); idf_max = max(idf_values)
    idf_mean = sum(idf_values) / len(idf_values)
    print(f"    corpus IDF: min={idf_min:.3f} mean={idf_mean:.3f} max={idf_max:.3f}")
    R.check("idf_has_variation", idf_max - idf_min > 0.1,
            f"range=[{idf_min:.3f},{idf_max:.3f}]")

    dev = next(m.parameters()).device
    tk = m.tok("Tell me the key ideas", return_tensors='pt')
    ids = tk['input_ids'].to(dev); mask_p = tk['attention_mask'].to(dev)
    with torch.no_grad():
        ctx = m.prepare_decode_context(ids, mask_p, update_stats=False)
    cb = ctx.content_bias[0].cpu()

    vals, idxs = cb.topk(20)
    top_idf = [corpus_idf.get(int(tid), 1.0) for tid in idxs.tolist() if vals[0].item() > 0]
    if top_idf:
        top_idf_mean = sum(top_idf) / len(top_idf)
        print(f"    top-20 biased tokens mean IDF={top_idf_mean:.3f}")
        R.check("idf_top_biased_above_corpus_mean",
                top_idf_mean >= idf_mean - 0.05,
                f"top-biased mean IDF {top_idf_mean:.3f} vs corpus {idf_mean:.3f}")

    if len(corpus_idf) >= 2:
        sorted_items = sorted(corpus_idf.items(), key=lambda x: x[1])
        common_tid, common_idf = sorted_items[0]
        rare_tid, rare_idf = sorted_items[-1]
        if rare_idf > common_idf + 0.1 and common_tid < m._wte_normed.shape[0] \
                and rare_tid < m._wte_normed.shape[0]:
            print(f"    common tid={common_tid} IDF={common_idf:.3f}; "
                  f"rare tid={rare_tid} IDF={rare_idf:.3f}")
            R.check("rare_token_gets_higher_idf_boost",
                    rare_idf > common_idf,
                    f"{rare_idf} !> {common_idf}")
    _clear(m)

def test_idf_bias_keyword_promotion(m, c, R):
    print("\n── [C-5] IDF content bias end-to-end on a music query ──")
    _write_mixed(m); m.eval()
    dev = next(m.parameters()).device

    tk = m.tok("The topic involves", return_tensors='pt')
    ids = tk['input_ids'].to(dev); mask_p = tk['attention_mask'].to(dev)
    with torch.no_grad():
        o_base = m.backbone(ids, mask_p)
        lg_base = o_base['logits'][:, -1, :].float()
        ctx = m.prepare_decode_context(ids, mask_p, update_stats=False)
        o_cond = m.fwd(ids, mask_p, ctx.prefix_cond)
        lg_cond = o_cond['logits'][:, -1, :]
        lg_uncond = None
        if ctx.prefix_uncond is not None:
            o_unc = m.fwd(ids, mask_p, ctx.prefix_uncond)
            lg_uncond = o_unc['logits'][:, -1, :]
        state = DecodeState()
        lg_shaped = m.shape_step_logits(
            lg_cond, lg_uncond, 0,
            ctx.content_bias, ctx.suppression_bias, ctx.vocab_bias, state)

    cc = m.content_classifier
    _, top_base = lg_base.topk(20)
    content_starters_base = sum(
        1 for t in top_base[0].tolist() if t in cc.content_starter_ids)
    _, top_shaped = lg_shaped.topk(20)
    content_starters_shaped = sum(
        1 for t in top_shaped[0].tolist() if t in cc.content_starter_ids)
    print(f"    top-20 content starters: base={content_starters_base} "
          f"shaped={content_starters_shaped}")
    R.check("idf_shaping_promotes_content_starters",
            content_starters_shaped >= content_starters_base,
            f"shaped {content_starters_shaped} < base {content_starters_base}")

    R.check("idf_shaping_adds_content_starter_signal",
            content_starters_shaped > 0,
            f"no content starters in top-20 after shaping")
    _clear(m)

def test_guidance_active_contract(m, c, R):
    print("\n── [C-4] guidance_active flag contract ──")
    dev = next(m.parameters()).device
    _write_mixed(m); m.eval()
    tk = m.tok("Tell me about piano music", return_tensors='pt')
    ids = tk['input_ids'].to(dev); mask_p = tk['attention_mask'].to(dev)
    with torch.no_grad():
        base = m.fwd(ids, mask_p)
        prefix_mem = m._get_prefix(base['hs'], mask_p, ids=ids)
    R.check("guidance_True_with_real_memory",
            _get_prefix_guidance(prefix_mem) is True)
    R.check("biases_attached_with_real_memory",
            getattr(prefix_mem, _PREFIX_CONTENT_BIAS_ATTR, None) is not None)
    _clear(m); m.eval()
    tk2 = m.tok("Hello world", return_tensors='pt')
    ids2 = tk2['input_ids'].to(dev); mask2 = tk2['attention_mask'].to(dev)
    with torch.no_grad():
        base2 = m.fwd(ids2, mask2)
        prefix_empty = m._get_prefix(base2['hs'], mask2, ids=ids2)
    R.check("guidance_False_with_empty_memory",
            _get_prefix_guidance(prefix_empty) is False)
    _write_mixed(m); m.eval()
    with torch.no_grad():
        ctx = m.prepare_decode_context(ids, mask_p, update_stats=False)
    R.check("guidance_False_on_ctx_path",
            _get_prefix_guidance(ctx.prefix_cond) is False)
    if ctx.prefix_uncond is not None:
        R.check("guidance_False_on_uncond",
                _get_prefix_guidance(ctx.prefix_uncond) is False)
    with torch.no_grad():
        neutral = m.bridge.build_neutral_prefix(1, dev)
    R.check("guidance_False_on_neutral_default",
            _get_prefix_guidance(neutral) is False)
    _clear(m)

def test_blank_vs_memory_differential(m, c, R):
    print("\n── 4.10 blank-vs-memory prefix differential ──")
    dev = next(m.parameters()).device
    _write_mixed(m); m.eval()
    tk = m.tok("Some piano question", return_tensors='pt')
    ids = tk['input_ids'].to(dev); mask_p = tk['attention_mask'].to(dev)
    with torch.no_grad():
        base_real = m.fwd(ids, mask_p)
        prefix_mem = m._get_prefix(base_real['hs'], mask_p, ids=ids)
    _clear(m); m.eval()
    with torch.no_grad():
        base_blank = m.fwd(ids, mask_p)
        prefix_blank = m._get_prefix(base_blank['hs'], mask_p, ids=ids)
    R.check("blank_prefix_guidance_is_False",
            _get_prefix_guidance(prefix_blank) is False)
    R.check("memory_prefix_guidance_is_True",
            _get_prefix_guidance(prefix_mem) is True)
    _write_mixed(m); m.eval()
    with torch.no_grad():
        o_no = m.fwd(ids, mask_p, None)
        o_blank = m.fwd(ids, mask_p, prefix_blank)
        o_mem = m.fwd(ids, mask_p, prefix_mem)
    lg_no = o_no['logits'][:, -1, :]
    lg_blank = o_blank['logits'][:, -1, :]
    lg_mem = o_mem['logits'][:, -1, :]
    blank_min = lg_blank.min().item()
    mem_min = lg_mem.min().item()
    R.check("blank_prefix_no_hard_mask_residue",
            blank_min > -1e5,
            f"blank min logit = {blank_min:.3e}")
    R.check("memory_prefix_has_hard_mask_in_early_step",
            mem_min < -1e5,
            f"memory min logit = {mem_min:.3e}")
    diff_blank_vs_no = (lg_blank - lg_no).abs().max().item()
    diff_mem_vs_blank = (lg_mem - lg_blank).abs().max().item()
    print(f"    max|Δ blank-vs-no|={diff_blank_vs_no:.3e} "
          f"max|Δ mem-vs-blank|={diff_mem_vs_blank:.3e}")
    R.check("differential_is_detectable",
            diff_mem_vs_blank > diff_blank_vs_no * 10,
            f"mem-vs-blank={diff_mem_vs_blank:.3e}, blank-vs-no={diff_blank_vs_no:.3e}")
    _clear(m)

def test_no_repeat_bigram_reduction(m, c, R):
    print("\n── [C-2] no_repeat_bigram reduces repeated_bigram_ratio ──")
    _write_mixed(m)
    total_ratio = 0.0; n_samples = 0
    prompts = ["The pianist", "Music theory", "The telescope", "Key piano ideas"]
    for seed in range(4):
        for p in prompts:
            torch.manual_seed(seed * 23 + 5)
            with torch.no_grad():
                gen = m.generate(p, mt=40, greedy=False)
            new_text = gen[len(p):].strip() if gen.startswith(p) else gen
            tok_ids = m.tok.encode(new_text)
            if len(tok_ids) < 4: continue
            bigrams = [(tok_ids[i], tok_ids[i+1]) for i in range(len(tok_ids)-1)]
            cnt = Counter(bigrams)
            repeated = sum(1 for _b, c_ in cnt.items() if c_ > 1)
            ratio = repeated / len(bigrams)
            total_ratio += ratio; n_samples += 1
    avg = total_ratio / max(n_samples, 1)
    print(f"    avg repeated_bigram_ratio across {n_samples} samples = {avg:.3f}")
    R.check("bigram_ratio_under_threshold", avg < 0.20, f"{avg:.3f} >= 0.20")
    _clear(m)

def test_runner_path_shaping_still_works(m, c, R):
    print("\n── [C-4] runner path + memory → shaping still active ──")
    _write_mixed(m); m.eval()
    dev = next(m.parameters()).device; cc = m.content_classifier
    prompts = ["Key piano ideas include", "The telescope"]
    viol = 0; total = 0
    for p in prompts:
        tk = m.tok(p, return_tensors='pt')
        ids = tk['input_ids'].to(dev); mask_p = tk['attention_mask'].to(dev)
        with torch.no_grad():
            base = m.fwd(ids, mask_p)
            prefix = m._get_prefix(base['hs'], mask_p, ids=ids)
            for step in range(c.early_starter_hard_mask_steps):
                o = m.fwd(ids, mask_p, prefix)
                lg = o['logits'][:, -1, :]
                nxt_id = lg.argmax(-1).item()
                total += 1
                if nxt_id not in cc.content_starter_ids:
                    viol += 1; break
                ids = torch.cat([ids, torch.tensor([[nxt_id]], device=dev)], 1)
                mask_p = torch.cat([mask_p, torch.ones(1, 1, device=dev, dtype=mask_p.dtype)], 1)
    R.check("runner_early_window_all_starters", viol == 0, f"{viol}/{total}")
    _clear(m)

def test_retrieval_purity(m, c, R):
    print("\n── retrieval purity ──")
    _write_mixed(m)
    dev = next(m.parameters()).device
    tk = m.tok("What improves piano technique and musical phrasing?", return_tensors='pt')
    ids, mask_p = tk['input_ids'].to(dev), tk['attention_mask'].to(dev)
    with torch.no_grad():
        ctx = m.prepare_decode_context(ids, mask_p, update_stats=False)
    diag = ctx.diag
    mw = sw = 0.0
    for mid, w in diag.batch_mem_weights[0]:
        if mid in m.amm.tree.store:
            text = m.amm.tree.store[mid].source_text.lower()
            if any(k in text for k in MUSIC_KEYS): mw += w
            elif any(k in text for k in SPACE_KEYS): sw += w
    print(f"    music_w={mw:.3f} space_w={sw:.3f}")
    R.check("music_dominates", mw >= sw * 2.0, f"music={mw:.3f} space={sw:.3f}")
    _clear(m)

def test_first_step_is_content_starter(m, c, R):
    print("\n── first-step content starter (generate path) ──")
    _write_mixed(m)
    cc = m.content_classifier
    prompts = ["Key piano ideas include", "Music theory is", "The pianist"]
    failures = 0; total = 0
    for p in prompts:
        for seed in range(3):
            torch.manual_seed(seed * 11 + 7)
            with torch.no_grad():
                gen = m.generate(p, mt=c.early_starter_hard_mask_steps + 2, greedy=False)
            prompt_tok_ids = m.tok.encode(p)
            full_tok_ids = m.tok.encode(gen)
            new_ids = full_tok_ids[len(prompt_tok_ids):]
            total += 1
            for k in range(min(c.early_starter_hard_mask_steps, len(new_ids))):
                if new_ids[k] not in cc.content_starter_ids:
                    failures += 1; break
    print(f"    generate-path violations: {failures}/{total}")
    R.check("generate_first_steps_are_starters", failures == 0)
    _clear(m)

def test_trainer_recon_public_api(m, c, R):
    print("\n── Trainer.recon public API ──")
    _clear(m)
    for t in ["The cat sat.", "Piano practice.", "Distant galaxies."]:
        m.write(t, training_mode=True)
    trainer = Trainer(m, c)
    R.check("recon_is_public_method", hasattr(trainer, 'recon') and callable(trainer.recon))
    result = trainer.recon("He played the piano softly.")
    R.check("recon_returns_dict", isinstance(result, dict))
    R.check("recon_has_loss", 'loss' in result and isinstance(result['loss'], torch.Tensor))
    R.check("recon_loss_finite", result['loss'].isfinite().item())
    R.check("recon_loss_has_grad", result['loss'].requires_grad)
    _clear(m)

def test_training_preserves_grad(m, c, R):
    print("\n── training-time shaping bypass safety ──")
    _clear(m)
    for t in ["The cat sat.", "Piano practice.", "Distant galaxies."]:
        m.write(t, training_mode=True)
    m.train()
    trainer = Trainer(m, c)
    r = trainer.recon("He played the piano softly.")
    R.check("train_recon_loss_has_grad_fn", r['loss'].grad_fn is not None)
    R.check("train_recon_loss_finite", r['loss'].isfinite().item())
    prefix = r['prefix']
    R.check("train_prefix_no_guidance_attr",
            _get_prefix_guidance(prefix) is False)
    r['loss'].backward()
    g_dir = m.amm.dir_pred.net[0].weight.grad
    R.check("train_grad_reaches_dir_pred",
            g_dir is not None and g_dir.abs().max().item() > 0)
    m.zero_grad(); m.eval(); _clear(m)

def test_generation_quality(m, c, R):
    print("\n── 生成质量 ──")
    _write_mixed(m)
    prompts = ["The pianist", "Key piano ideas", "What improves piano technique?"]
    total = 0; healthy_alpha = 0; healthy_len = 0
    for p in prompts:
        for seed in range(2):
            torch.manual_seed(seed * 17 + 3)
            with torch.no_grad():
                gen = m.generate(p, mt=30, greedy=False)
            new = gen[len(p):].strip() if gen.startswith(p) else gen
            total += 1
            alpha = sum(1 for ch in new if ch.isalpha())
            if alpha / max(len(new), 1) > 0.6: healthy_alpha += 1
            if len(new) >= 15: healthy_len += 1
    print(f"    samples={total} alpha={healthy_alpha} len={healthy_len}")
    R.check("gen_mostly_alpha", healthy_alpha >= int(total * 0.6))
    R.check("gen_nonempty", healthy_len >= int(total * 0.75))
    _clear(m)
...
def test_empty_memory(m, c, R):
    ...
    with torch.no_grad():
        ctx = m.prepare_decode_context(ids, mask_p, update_stats=False)
    R.check("empty_mem_prefix_finite", ctx.prefix_cond.isfinite().all().item())
    with torch.no_grad(): gen = m.generate("Hello", mt=6, greedy=True)
...
def test():
    ...
    print(f"  参数: 总{total:,}  可训练{train_p:,}")
    print(f"  d_LLM={c.d_LLM}  vocab={c.vocab_size}  n_layers={m.backbone.n_layers}")
    test_backbone(m, c, R)
    test_hungarian(m, c, R)
    test_directiontree_api(m, c, R)
    test_query_context_capture_hook(m, c, R)
    test_tree_semantic_rerank(m, c, R)
    test_tree_rerank_training_bypass(m, c, R)
    test_tree_rerank_preserves_signature(m, c, R)
    test_idf_content_bias(m, c, R)
    test_idf_bias_keyword_promotion(m, c, R)
    test_guidance_active_contract(m, c, R)
    test_blank_vs_memory_differential(m, c, R)
    test_runner_path_shaping_still_works(m, c, R)
    test_no_repeat_bigram_reduction(m, c, R)
    test_retrieval_purity(m, c, R)
    test_first_step_is_content_starter(m, c, R)
    test_trainer_recon_public_api(m, c, R)
    test_training_preserves_grad(m, c, R)
    test_generation_quality(m, c, R)
