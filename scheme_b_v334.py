...
[B-1] MemLLM.fwd() 在 (not training) + (prefix is not None) 时,对 backbone
      logits 的最后一个位置施加 F-2 content-starter hard mask。runner 走
      _get_prefix() + fwd(ids, mask, prefix) 的路径因此自动受益。

[B-2] _get_prefix() 把 prompt_length 作为属性 (_mem_decode_prompt_len)
      绑定到返回的 prefix 张量,fwd() 据此推导 step = ids.shape[1] - prompt_len.

[B-3] Hard mask 值用 -1e9(不是 -inf),保证 runner 手写 CFG 算术不爆 NaN;
      分析了 3 种对称/非对称情况,case (cond 无 mask, uncond 有 mask) 反语义
      且不自然,记录为已知边界。

[B-4] self.training == True 时跳过 hard mask,保护 _recon_forward 的梯度。

[B-5] DirectionTree.max_depth() / leaf_size_violations() 作为真实公共 API 加回。

保留 v3.33 的 [A-1] shape_step_logits / [A-2] prepare_decode_context /
       [A-3] Trainer.recon / [A-4] DecodeState —— 都是合格的 public API。
"""
...
class DirectionTree:
    ...
    # ─── [B-5] Real public APIs (not runner-side shims) ───
    ...
    def leaf_size_violations(self) -> int:
        """Count leaves whose size exceeds cfg.tree_max_leaf."""
        cnt = [0]
        def _check(nd):
            if nd.leaf:
                if len(nd.ids) > self.c.tree_max_leaf: cnt[0] += 1
            else:
...
def _set_prefix_meta(prefix_tensor, prompt_len):
    try:
        setattr(prefix_tensor, _PREFIX_META_ATTR, int(prompt_len))
    except Exception:
        pass
...
class MemLLM(nn.Module):
    ...
    def fwd(self, ids, mask, prefix=None):
        ...
        if (prefix is None or self.training
                or not self.c.use_fwd_path_hard_mask
                or not self.c.use_early_content_starter_hard_mask
                or self.content_classifier is None):
            return out
        prompt_len = _get_prefix_meta(prefix)
        if prompt_len is None: return out
        step = int(ids.shape[1]) - int(prompt_len)
        if step < 0 or step >= self.c.early_starter_hard_mask_steps: return out
        logits = out['logits']; dev = logits.device
        starter_mask = self.content_classifier.content_starter_mask(dev)
        V = min(logits.shape[-1], starter_mask.shape[0])
        mask_val = float(self.c.fwd_path_hard_mask_value)
        last = logits[:, -1:, :V]
        mask_bool = starter_mask[:V].bool().view(1, 1, V)
        last_masked = torch.where(mask_bool, last, torch.full_like(last, mask_val))
        new_logits = logits.clone()
        new_logits[:, -1:, :V] = last_masked
