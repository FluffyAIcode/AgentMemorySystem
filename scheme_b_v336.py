...
[C-4] _mem_guidance_active 语义闭包,修复 4.10 差分漂移
      v3.35 的 fwd() 无条件对带 prompt_len 的 prefix 施加 early hard mask + bias,
      导致 runner 的 blank-memory vs memory-prefix 差分被 -1e9 hard mask 淹没
      (L2 shift 两边都是 ~3.2e11,差分信号不可见)。

      v3.36 引入显式 guidance_active 标记:
      - _get_prefix(return_extra=False) + 有效记忆 → True
      - _get_prefix(return_extra=True) → False (ctx 路径,由 shape_step_logits 处理)
      - build_neutral_prefix / _build_contrastive_uncond_prefix → False
      - 空记忆 / retrieval 返回 empty_state / 全被 gate 丢弃 → False

      fwd() 检查该标记:False 时纯 backbone 透传,不施加任何 shaping。
      这消除了 4.10 的结构性冲突,同时保持 4.12/4.15 的 runner-path shaping。

      附带收益:generate() 路径不再有 fwd/shape_step_logits 双重 hard mask。

保留 v3.35 的 [C-1/C-2/C-3] 和 v3.33/v3.34 的 [A-*]/[B-*]。
"""
...
class Cfg:
    ...
    # [C-4] guidance detection threshold
...
class MemLLM(nn.Module):
    ...
    def _get_prefix(self, hs, mask=None, pl=0, update_stats=True, return_extra=False, ids=None):
        ...
        if return_extra:
            # ctx-path: shape_step_logits handles all shaping.
            # fwd() must be a pure backbone pass → guidance=False, no biases attached.
        ...
        # Runner-direct path: gate on actual retrieval content.
    ...
    def _build_contrastive_uncond_prefix(self, diag, prefix_cond, prompt_len_for_meta=None):
        ...
        # CFG contrast branch: fwd() must not apply shaping.
