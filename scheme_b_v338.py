...
═══════════════════════════════════════════════════════════════════════════
修复相对 v3.37:
[D-1] L_functional_suppression:训练时 margin-style 损失,强制前缀
      使 max(top_content_starter_logit) > max(top_functional_logit) + margin.
      修复 4.7 / 4.10 声量不足 (content_bias 上限 ~5 < 功能词 gap ~8).
[D-2] Rare-keyword tail slot:tail_semantic_anchor_loss 对 slot[0] 用
      uniform-content KL,对 slot[1] 用 IDF-top-K strict-starter KL.
      修复 4.15 inject 稀有域词进不了桥表达谱的问题.
[D-3] Context descriptor:从检索到的记忆 semantic_emb 加权聚合出
      d_LLM 维语境向量,经 context_head 替换 prefix 倒数第三 slot.
      修复 4.6 歧义消解失败 (C-6 找对记忆但 Qwen 跑题).
[D-4] Anti-collapse:per-token 历史衰减 content_bias, 并在 unique-ratio
      低于阈值时整体 dampen 所有 bias. 修复 4.8 自激振荡.

保留 v3.37 的 [C-1..C-6] 全部结构.
"""
...
class Cfg:
    ...
    def __post_init__(self):
        ...
        assert 0.8 < rr < 1.2
        # [D-2/D-3] slot budget sanity: body + tail + context <= L_mem
        ctx_slots = 1 if (self.use_context_descriptor and self.context_slot_enabled) else 0
        used = self.content_tail_slots + ctx_slots
        assert used < self.L_mem, f"tail+ctx={used} must be < L_mem={self.L_mem}"
...
class ContentSemanticTailHead(nn.Module):
    """
    [D-2] n_slots=2: slot[0]=general content direction, slot[1]=rare keyword direction.
    """
...
class ContextHead(nn.Module):
    """[D-3] Projects aggregated semantic_emb (d_LLM) to a single prefix slot (d_LLM)."""
...
class ContentTokenClassifier:
    ...
    def function_mask(self, device):
        """[D-1] function_ids as a dense mask (punct/stopwords/short-tokens)."""
...
class MemEntry:
    ...
    # [D-2] sorted list of IDF-top-K strict starter ids for this memory
...
class EmbBridge(nn.Module):
    def __init__(self, c):
        ...
        # [D-3] context slot head
        if c.use_context_descriptor and c.context_slot_enabled:
            self.context_head = ContextHead(c.d_LLM)
        else:
    ...
    def inject(self, fibers, mem_mask=None, fiber_summary=None,
               filler_centroid=None, context_descriptor=None):
        """
        Slot layout (from front to back):
          [ body ... ] [ context? ] [ tail_0: general ] [ tail_1: rare ]
        """
        ...
        ctx_slot_used = 0
        pieces = []
        use_ctx = (self.c.use_context_descriptor and self.c.context_slot_enabled
                   and self.context_head is not None and context_descriptor is not None)
        if use_ctx:
            ctx_emb = self.context_head(context_descriptor)
            ctx_emb_aligned = self.aligner(ctx_emb.unsqueeze(1))
            pieces.append(ctx_emb_aligned)
            ctx_slot_used = 1
            self._last_context_slot = ctx_emb_aligned.detach()
        else:
            self._last_context_slot = None
        if (self.c.use_content_semantic_tail and self.c.content_tail_slots > 0
            ...
            tail_slots_used = self.c.content_tail_slots
            self._last_tail_slots = tail_aligned.detach()
        else:
            self._last_tail_slots = None
        n_replace = ctx_slot_used + tail_slots_used
...
class MemLLM(nn.Module):
    ...
    def _compute_rare_keyword_ids(self, mem, corpus_idf):
        """[D-2] Select top-K IDF-weighted strict starters for a memory."""
    ...
    def _compute_context_descriptor(self, diag):
        """[D-3] Weighted-average of retrieved memory semantic_emb vectors."""
        ...
        for b in range(B):
            ...
            ctx_sum = torch.zeros(self.c.d_LLM, device=dev)
            w_sum = 0.0
            for mid, w in mw:
                if mid in self.amm.tree.store and w > 0:
                    mem = self.amm.tree.store[mid]
                    if mem.semantic_emb is not None:
                        ctx_sum = ctx_sum + w * mem.semantic_emb.to(dev).float()
                        w_sum += w
            if w_sum > 1e-6:
                result.append(ctx_sum / w_sum); any_populated = True
            else:
                result.append(torch.zeros(self.c.d_LLM, device=dev))
    ...
    def _get_prefix(self, hs, mask=None, pl=0, update_stats=True, return_extra=False, ids=None):
        ...
        # [D-3] Compute context descriptor from retrieved memories
        ctx_desc = (self._compute_context_descriptor(diag)
    ...
    def shape_step_logits(self, logits_cond, logits_uncond, step,
        ...
        # [D-4] degeneration detector: dampen overall bias magnitude when repeating
...
class Trainer:
    def __init__(self, m, c):
        ...
        if m.bridge.context_head is not None:
            self.grad_monitor.register('context_head', m.bridge.context_head)
    ...
    def tail_semantic_anchor_loss(self, fiber, ids, mask):
        """[D-2] Dual-target KL: slot[0] general, slot[1] rare IDF-top-K."""
    ...
    def functional_suppression_loss(self, prefix, ids, mask):
        """[D-1] Hinge: best content-starter logit must exceed best functional-token logit."""
        ...
        func_mask = cc.function_mask(dev)[:V_cur].clone().bool()
        nl_mask = torch.zeros(V_cur, dtype=torch.bool, device=dev)
        for t in cc.newline_ids:
            if t < V_cur: nl_mask[t] = True
        func_mask = func_mask & (~nl_mask)
        pt_mask = torch.zeros(V_cur, dtype=torch.bool, device=dev)
        for t in cc.punct_ids:
            if t < V_cur: pt_mask[t] = True
        func_mask = func_mask & (~pt_mask)
        eos_id = self.m.tok.eos_token_id
        if eos_id is not None and eos_id < V_cur:
            func_mask[eos_id] = False
