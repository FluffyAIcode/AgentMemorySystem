...
修复相对 v3.31 (基于 report.md 反馈):

[F-1] Memory-Guided Contrastive Logit Suppression
      non-dominant memory 的 content tokens 做负 bias
      解决: music_output == space_output (CFG 不够,需要对称 contrast)

[F-2] Early-Step Content Starter Hard Mask
      early_content_steps 内 硬 mask 非 content-starter (不是软惩罚)
      解决: step 0 top1=punct

[F-3] Adaptive Memory Bias Scaling
      content_bias 尺度 = base_scale * logits.std (自适应)
      解决: Qwen logits 动态范围大, 绝对 scale 不够

[F-4] Retrieval 阈值按 Qwen embedding 分布重标定
      strict_overlap_sim_threshold: 0.45 → 0.32
      ret_dir_weight: 0.05 → 0 (合并进 centroid/bidi)

[F-5] Bypass gate default OPEN
      bypass_init_gate_bias: -0.5 → 0.5

[F-6] CFG scale: 2.0 → 3.5

所有 v3.31 的核心机制保持:
      [S-1] Mean-Centered Contrastive Retrieval Scoring
      [S-2] Hungarian bipartite matching
      [S-3] Contrastive-Memory CFG
      [S-4] Content-Semantic Tail Head
      [B]   LLMBackbone 抽象
"""
...
class Cfg:
    ...
    # [F-2] Early-step HARD mask
    early_content_steps: int = 5                              # ↑
    use_early_content_starter_hard_mask: bool = True          # 新
    early_starter_hard_mask_steps: int = 3                    # 新: 前 N 步 hard mask 非 starter

    # [F-3] Adaptive content_bias scale
    content_bias_scale: float = 6.0                           # 基础 scale
    use_adaptive_content_bias_scale: bool = True              # 新: 按 logits.std 缩放
    content_bias_std_multiplier: float = 1.5                  # 新: content_bias ~ mul * logits.std
    content_bias_decay: float = 0.02                          # ↓慢衰减
    content_bias_floor: float = 0.5                           # ↑高底线
    ...
    # [F-1] Contrastive Logit Suppression
    use_memory_guided_suppression: bool = True                # 新
    suppression_bias_scale: float = 4.0                       # 新
    suppression_std_multiplier: float = 1.0                   # 新
    suppression_decay: float = 0.03                           # 新
    suppression_floor: float = 0.3                            # 新
    ...
    # [F-4] retrieval weights (dir=0)
    ret_centroid_weight: float = 0.30                         # ↑
    ret_sem_weight: float = 0.10
    ret_bidi_min_weight: float = 0.25                         # ↑
    ret_forward_maxsim_weight: float = 0.35                   # 微调
    ret_dir_weight: float = 0.00                              # ↓ 合并进 centroid
    ...
    # [F-4] Qwen embedding distribution → lower threshold
...
class LLMBackbone(nn.Module):
    ...
    def to(self, *args, **kwargs):
        ...
        if 'device' in kwargs:
            self._wte_fp32 = self._wte_fp32.to(kwargs['device'])
        return self

    def forward(self, ids: torch.Tensor, attention_mask: torch.Tensor,
                prefix: Optional[torch.Tensor] = None) -> Dict:
...
class PrefixAligner(nn.Module):
    ...
    def calibrate(self, wte_fp32: torch.Tensor):
        with torch.no_grad():
            V = wte_fp32.shape[0]
            si = min(5000, V)
...
class ContentSemanticTailHead(nn.Module):
    def __init__(self, d_F: int, d_LLM: int, n_slots: int, hidden: int = 1024):
        super().__init__()
...
class ContentTokenClassifier:
    ...
    def __init__(self, tokenizer, cfg=None, vocab_size=None,
                 min_len=None, strict_min_len=None):
...
class DirectionTree:
    ...
    def max_depth(self):
        def _d(nd):
            ...
            return max(_d(c) for c in nd.children) if nd.children else nd.depth
        return _d(self.root)
    def leaf_size_violations(self)->List[Tuple[int,int]]:
        vios=[]
        def _walk(nd):
            if nd.leaf:
                if len(nd.ids)>self.c.tree_max_leaf: vios.append((nd.depth,len(nd.ids)))
            else:
                for c in nd.children: _walk(c)
        _walk(self.root); return vios
...
class AMM(nn.Module):
    ...
    def _compute_forward_hungarian(self, query_ids, mem_ids, wte_normed,
    ...
    def retrieve_multi(self, xq, fq, topk=None, bw=None, update_stats=True,
        ...
        for b in range(B):
            ...
            if self.c.use_idf_centroid and q_content_ids and wn is not None:
                q_centroid = self._compute_idf_weighted_centroid(
                    q_content_ids, wn, corpus_idf, idf_floor)
            ...
            dominant_mid: Optional[int] = None
            non_dominant_mids: List[int] = []
            non_dom_weights: Dict[int, float] = {}
            if C >= 1:
...
class MemLLM(nn.Module):
    def __init__(self, c):
        ...
        self.layer_pool = None; self.backbone: Optional[LLMBackbone] = None
        ...
        self._filler_centroid: Optional[torch.Tensor] = None
    ...
    def _build_wte_neighbor_cache(self):
        ...
        if V > self.c.wte_neighbor_max_vocab:
            ...
            print(f"  [neighbor cache] vocab_size={V} > {self.c.wte_neighbor_max_vocab}, skip build")
            return
    ...
    def _build_token_bias_from_memories(self, mem_weight_list, q_content_ids):
        """构造一个 (V,) 的 token bias. 输入:
           mem_weight_list = [(mid, weight), ...]
           q_content_ids = list of content token ids from query
        """
    ...
    def _build_content_bias(self, diag, query_content_ids_per_batch):
        """Positive bias (dominant-memory content tokens)."""
        V = self.c.vocab_size; dev = next(self.parameters()).device
    ...
    def _build_suppression_bias(self, diag, query_content_ids_per_batch):
        """[F-1] Negative bias from non-dominant memories.
           Returns (B, V) tensor, to be SUBTRACTED from logits.
           Only tokens present in non-dominant memories but NOT in dominant memory are suppressed.
        """
        V = self.c.vocab_size; dev = next(self.parameters()).device
        B = len(diag.batch_mem_weights)
        ...
        for b in range(B):
            ...
            # Dominant content tokens set (no suppression for these)
            dom_token_set: Set[int] = set()
            ...
            # Zero out tokens that also belong to dominant memory
            for t in dom_token_set:
                if 0 <= t < V:
                    nd_bias[t] = 0.0
            nmax = nd_bias.max()
            if nmax > 1e-8:
                suppression[b] = nd_bias / nmax
        return suppression
        ...
        for b in range(B):
            if have_contrast[b]:
                ...
                pref_b = self.bridge.inject(
                    single, mask_one,
                    fiber_summary=non_dom_fibers_t[b:b+1],
                    filler_centroid=self._filler_centroid)
    ...
    def _prep_prompt_ids(self, prompt: str):
    ...
    def generate(self, prompt, mt=50, greedy=False):
        ...
        with torch.no_grad():
            o = self.fwd(ids, mask)
            prefix_cond, fiber_summary, diag, content_bias, suppression_bias = self._get_prefix(
            ...
            if self.c.use_cfg_decoding:
                if self.c.use_contrastive_memory_cfg:
                    prefix_uncond = self._build_contrastive_uncond_prefix(diag, prefix_cond)
                else:
                    B = prefix_cond.shape[0]
                    prefix_uncond = self.bridge.build_neutral_prefix(B, dev)
            else: prefix_uncond = None
        generated_ids = []
        ...
        for i in range(mt):
            ...
            with torch.no_grad():
                ...
                # [F-3] Adaptive scaling of content_bias / suppression_bias
                if self.c.use_adaptive_content_bias_scale:
                    logits_std = lg.std().item()
                    cb_unit = logits_std * self.c.content_bias_std_multiplier
                    sup_unit = logits_std * self.c.suppression_std_multiplier
                else:
                    cb_unit = 1.0
                    sup_unit = 1.0

                step_scale_cb = max(self.c.content_bias_floor,
                ...
                if content_bias is not None and content_bias.abs().max().item() > 0.01:
                    ...
                    lg[:, :V] = lg[:, :V] + content_bias[:, :V] * cb_unit * self.c.content_bias_scale * step_scale_cb

                step_scale_sup = max(self.c.suppression_floor,
                                     1.0 - i * self.c.suppression_decay)
                if (self.c.use_memory_guided_suppression
                        and suppression_bias is not None
                        and suppression_bias.abs().max().item() > 0.01):
                    V = min(V_lg, suppression_bias.shape[-1])
                    lg[:, :V] = lg[:, :V] - suppression_bias[:, :V] * sup_unit * self.c.suppression_bias_scale * step_scale_sup
                ...
                if cc:
                    for tid, count in generated_content_counts.items():
                        if tid in cc.content_ids and tid < V_lg:
                            scaled_count = count ** self.c.content_repeat_exponent
                            lg[0, tid] -= self.c.content_repeat_penalty * scaled_count

                if self.c.use_cyclic_content_hard_mask and cc is not None:
                    window = self.c.cyclic_content_window
                    max_cnt = self.c.cyclic_content_max_count
                    window_counts: Dict[int, int] = {}
                    cutoff_step = i - window
                ...
                if self.c.use_newline_hard_gate and cc is not None:
                    ...
                    hard_gate_active = (
                        i < self.c.newline_hard_gate_min_step
                        or content_count_so_far < self.c.newline_hard_gate_min_content)
                    if hard_gate_active:
                        for nid in newline_ids_set:
                            if nid < V_lg: lg[0, nid] = HARD_MASK

                if (self.c.use_eos_hard_mask and eos_token_id is not None
                        and i < self.c.eos_hard_mask_steps and eos_token_id < V_lg):
                ...
                if self.c.use_content_gated_newline and cc is not None:
                    ...
                    if content_count_so_far < self.c.min_content_tokens_before_newline:
                        for nid in newline_ids_set:
                            if nid < V_lg: lg[0, nid] -= self.c.late_newline_penalty

                # [F-2] Early-step HARD mask (非 content-starter 全部 mask)
                if (self.c.use_early_content_starter_hard_mask and cc is not None
                        and i < self.c.early_starter_hard_mask_steps):
                    starter_mask = cc.content_starter_mask(dev)[:V_lg]
                    # HARD mask: logits = -inf for non-starters
            ...
            recent_starters = [(t, s) for (t, s) in recent_starters
                               if (i - s) < self.c.bpe_echo_window]
