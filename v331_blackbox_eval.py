...
def mixture_distribution_gate_probe(seed: int) -> Dict[str, Any]:
    ...
    The SUT must expose a per-token gate tensor on DecodeContext. v3.38 does
    not; the probe reports not_implemented per spec."""
    model = build_model(seed)
    write_texts(model, corpus_music())
    device = next(model.parameters()).device
    ...
    with torch.no_grad():
        ctx = model.prepare_decode_context(ids, mask, update_stats=False)
    # Look for a mixture gate on DecodeContext.
    gate = getattr(ctx, "mixture_gate", None)
    if gate is None:
        return {
            "passed": False,
            "status": "not_implemented",
            "missing_api": "DecodeContext.mixture_gate (expected shape [B, V] in [0,1])",
            "note": ("v3.38 shapes logits additively via content_bias/"
                     "suppression_bias/CFG; no convex mixture gate is exposed."),
            "gating": "PASS_or_not_implemented",
        }
    # Were this implemented, we'd verify the decomposition and boundary behavior.
    return {
        "passed": False,
        "status": "not_implemented",
        "missing_api": "verification path for mixture_gate is defined but "
                       "gate tensor present; decomposition check not implemented "
                       "in this runner. Emit not_implemented to avoid false positive.",
        "gating": "PASS_or_not_implemented",
    }
