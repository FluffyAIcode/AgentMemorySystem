...
═══════════════════════════════════════════════════════════════════════════
相对 v3.38 的结构性修复:
[E-1] MemEntry.context_descriptor: 写入时用 ContextEncoder 投影 d_LLM→d_ctx,
      存入每条记忆; retrieval 时聚合 per-memory descriptor (而非 semantic_emb).
      修复 4.24 spec missing_api.
[E-2] upstream_gate_min_keep_for_rerank: 上游门后保留 >=3 个候选,保证
      rerank 稳定性探针能计算 Spearman. 修复 4.20.
[E-3] 解码时主动功能词压制 (decode_functional_suppression):
      在 shape_step_logits 中,当前步 top function token 比 top starter
      高出 margin 时,对所有 function tokens 施加 logit 惩罚. 这是与
      [D-1] 训练损失正交的 decode-time 机制. 修复 4.22 eval-only FAIL.
[E-4] WTE-residual tail slot[1]: tail 的 rare slot = learned_head_output
      + alpha * Aligner(rare_keyword_WTE_centroid_projection).
      α 项提供 architectural guarantee: 未训练时 slot[1] 已经指向正确方向.
      修复 4.23 eval-only FAIL.
[E-5] scale_tail_with_L_mem: tail/ctx slot 数量按比例缩放 L_mem,
      保证扩容不退化为同质 body slots. 修复 4.25.
[E-6] 新增 convex mixture 解码模式: 训练一个 mixture_gate 网络, 在 decode
      时每步做 (1-g)*logit_cond + g*logit_memory. DecodeContext.mixture_gate
      字段暴露给审计. 修复 4.26.
"""
...
class Cfg:
    ...
    # [E-2] Ensure rerank has enough candidates for stability
    upstream_gate_min_keep_for_rerank: int = 3
    ...
    # [E-2] Ensure strict-overlap gate also preserves candidates for rerank
    strict_overlap_min_keep_for_rerank: int = 3
    ...
    # [E-1] per-memory context descriptor
    use_memory_context_encoder: bool = True
    d_ctx: int = 128
    context_encoder_hidden: int = 256
    # [E-3] decode-time functional suppression (structural, not training)
    ...
    # [E-4] WTE-residual tail slot (architectural guarantee for rare keyword)
    use_wte_residual_tail: bool = True
    wte_residual_alpha: float = 0.6
    # [E-5] scale tail/ctx slots with L_mem
    scale_tail_with_L_mem: bool = True
    tail_L_mem_ratio: int = 4   # tail_slots = max(content_tail_slots, L_mem // ratio)
    ctx_L_mem_threshold: int = 12  # additional ctx slot when L_mem >= threshold
    # [E-6] convex mixture decode mode
    ...
    def effective_tail_slots(self) -> int:
        ...
        if self.scale_tail_with_L_mem and self.tail_L_mem_ratio > 0:
            scaled = self.L_mem // self.tail_L_mem_ratio
    ...
    def __post_init__(self):
        ...
        assert used < self.L_mem, \
            f"effective tail({tail_eff})+ctx({ctx_eff})={used} must be < L_mem={self.L_mem}"
        ...
        assert 0.0 <= self.wte_residual_alpha <= 1.0
...
class ContentSemanticTailHead(nn.Module):
    """[D-2 + E-4] Dual-slot tail with optional WTE residual addition on slot[1]."""
    ...
    def forward(self, fiber_summary, wte_residuals=None):
        if self.n_slots == 0: return None
        h = self.shared(fiber_summary)
        slots = [head(h) for head in self.slot_heads]
        out = torch.stack(slots, dim=1)
        if wte_residuals is not None:
            out = out + wte_residuals
        return out

class ContextHead(nn.Module):
    """[D-3] Projects aggregated d_LLM-space context to prefix slot."""
...
class MemoryContextEncoder(nn.Module):
    """[E-1] Per-memory context_descriptor encoder. d_LLM → d_ctx (normalized)."""
    def __init__(self, d_LLM, d_ctx, hidden=256):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(d_LLM, hidden), nn.SiLU(), nn.LayerNorm(hidden),
    ...
    def encode(self, hidden_mean):
        return F.normalize(self.net(hidden_mean), dim=-1, eps=1e-8)
    def decode(self, ctx_vec):
        return self.back_proj(ctx_vec)

class MixtureGateHead(nn.Module):
    """[E-6] Computes a gate g in [floor, ceiling]."""
...
class ContentTokenClassifier:
    ...
    def pure_function_mask(self, device, eos_id=None):
        """[E-3] function_ids minus newlines, punct, and EOS."""
        cache_key = (device, eos_id)
        if (self._pure_function_tensor is None or self._pf_key != cache_key):
            V = self._mask_size(); m = torch.zeros(V, device=device)
...
class MemEntry:
    ...
    # [E-1] per-memory context descriptor in d_ctx space (spec-compliant)
...
class EmbBridge(nn.Module):
    ...
    def inject(self, fibers, mem_mask=None, fiber_summary=None,
        ...
        if (self._effective_tail_slots > 0 and fiber_summary is not None):
            tail = self.tail_head(fiber_summary, wte_residuals=rare_keyword_wte_residual)
...
class RetrievalDiag:
    ...
    # [E-2] track final candidate count available to rerank
...
class AMM(nn.Module):
    ...
    def store_mem(self, h, surp, training_mode=False, source_text="",
                ...
                if mid in self.tree.store:
                    ...
                    if dist<self.c.consol_dist*self.c.consol_conflict_ratio:
                        ...
                        if context_descriptor is not None:
                            if ex.context_descriptor is not None:
                                merged = ((1-alpha)*ex.context_descriptor.to(dev)
                                          + alpha*context_descriptor)
                                ex.context_descriptor = F.normalize(
                                    merged, dim=-1, eps=1e-8).detach().clone()
                            else:
                                ex.context_descriptor = context_descriptor.detach().clone()
                        ex.rare_keyword_ids = []
        ...
        m=MemEntry(mid=self.tree.nid,base=x.detach().clone(),fiber=f.detach().clone(),
                   ...
                   context_descriptor=(context_descriptor.detach().clone()
                                        if context_descriptor is not None else None))
        self.tree.nid+=1; self.tree.insert(m); self.time+=1; return m

    def _apply_min_keep_for_rerank(self, candidates_count, min_keep_spec,
                                   min_keep_rerank, pass_mask, fallback_scores):
        """[E-2] Ensure at least min_keep_rerank candidates survive when available."""
        effective_min = max(min_keep_spec, min_keep_rerank)
        effective_min = min(effective_min, candidates_count)
        n_pass = int(pass_mask.sum().item())
        if n_pass >= effective_min: return pass_mask
        keep_n = effective_min
        top_keep = fallback_scores.topk(min(keep_n, candidates_count)).indices
        new_mask = torch.zeros_like(pass_mask)
        new_mask[top_keep] = True
        return new_mask | pass_mask
    ...
    def retrieve_multi(self, xq, fq, topk=None, bw=None, update_stats=True,
        ...
        for b in range(B):
            ...
            if (self.c.use_strict_content_overlap_gate
                ...
                # [E-2] preserve enough for rerank
                pass_mask = self._apply_min_keep_for_rerank(
                    len(mems),
                    self.c.strict_overlap_min_keep,
                    self.c.strict_overlap_min_keep_for_rerank,
                    pass_mask,
                    overlap_counts.float())
            ...
            if self.c.use_upstream_semantic_gate and q_content_ids and wn is not None:
                ...
                # [E-2] ensure rerank has >=3 candidates
                pass_mask = self._apply_min_keep_for_rerank(
                    C_init,
                    self.c.upstream_gate_min_keep,
            ...
            diag.n_candidates_for_rerank = C_init  # [E-2]
            sb = sb_all; sf = sf_all
        ...
        for i in range(len(ms)):
            ...
            for j in range(i+1, len(ms)):
                ...
                if d < self.c.consol_dist:
                    ...
                    if ms[i].context_descriptor is not None and ms[j].context_descriptor is not None:
                        ctx_merged = (ms[i].context_descriptor*wi + ms[j].context_descriptor*wj) / t
                        ms[i].context_descriptor = F.normalize(ctx_merged, dim=-1, eps=1e-8).detach().clone()
                    elif ms[j].context_descriptor is not None:
                        ms[i].context_descriptor = ms[j].context_descriptor.clone()
...
class DecodeContext:
    ...
    # [E-6] convex mixture decode fields
...
class MemLLM(nn.Module):
    def __init__(self, c):
        ...
        # [E-1] per-memory context encoder
    ...
    def _compute_rare_keyword_ids(self, mem, corpus_idf):
        ...
        ranked = sorted(candidates,
                        key=lambda t: -corpus_idf.get(t, self.c.idf_floor))
    ...
    def _compute_aggregated_context_descriptors_d_llm(self, diag):
        """[E-1 + E-5] Aggregate per-memory context_descriptor to K d_LLM vectors."""
    ...
    def _compute_rare_keyword_wte_residual(self, diag):
        """[E-4] (B, n_tail_slots, d_LLM) residual; slot[1] = alpha * WTE centroid of rare keywords."""
        if not self.c.use_wte_residual_tail:
            return None
        if self.bridge._effective_tail_slots < 2:
            return None
        ...
        for b in range(B):
            ...
            for mid, w in mw:
                ...
                mem = self.amm.tree.store[mid]
                for tid in mem.rare_keyword_ids:
                    if tid < V_wte:
                        kw_weights[tid] = kw_weights.get(tid, 0.0) + w
            if not kw_weights: continue
            ids = list(kw_weights.keys())
            weights = torch.tensor([kw_weights[t] for t in ids], device=dev, dtype=wte_fp32.dtype)
            weights = weights / weights.sum().clamp(min=1e-8)
            vecs = wte_fp32[torch.tensor(ids, device=dev)]
            centroid = (vecs * weights.unsqueeze(1)).sum(0)
            target_std = self.bridge.aligner._target_std.item()
            target_norm = target_std * math.sqrt(self.c.d_LLM)
            cen_norm = centroid.norm().clamp(min=1e-8)
            centroid_scaled = centroid * (target_norm / cen_norm)
            residual[b, 1, :] = alpha * centroid_scaled
            any_nonzero = True
        if not any_nonzero: return None
        return residual

    def _compute_mixture_memory_logit(self, fiber_summary, diag, ids, mask):
        """[E-6] memory-proposed logit distribution for convex mixture."""
    ...
    def shape_step_logits(self, logits_cond, logits_uncond, step,
        ...
        # [E-3] Active decode-time functional suppression
    ...
    def write(self, text, training_mode=False):
        ...
        # [E-1] compute per-memory context_descriptor
        ctx_desc_batch = None
        if self.memory_context_encoder is not None:
            with torch.no_grad():
                ctx_desc_batch = self.memory_context_encoder.encode(content_sem)
        stored = 0; gate_vals = []
        for b in range(ids.shape[0]):
            with torch.no_grad():
            ...
            if training_mode or gate >= self.c.write_gate_threshold:
                ctx_desc = (ctx_desc_batch[b] if ctx_desc_batch is not None else None)
    ...
    def save_memory(self, path):
        ...
        for mid, m in self.amm.tree.store.items():
            data['store'][mid] = {
                ...
                'rare_keyword_ids': m.rare_keyword_ids,
                'semantic_emb': m.semantic_emb.cpu() if m.semantic_emb is not None else None,
                'context_descriptor': (m.context_descriptor.cpu()
                                        if m.context_descriptor is not None else None)}
    ...
    def load_memory(self, path):
        ...
        for mid, d in data['store'].items():
            ...
            ctx = d.get('context_descriptor', None)
            if ctx is not None: ctx = ctx.to(dev)
            m = MemEntry(mid=mid, base=d['base'].to(dev), fiber=d['fiber'].to(dev),
                ...
                rare_keyword_ids=d.get('rare_keyword_ids', []),
                semantic_emb=sem,
...
class Trainer:
    ...
    def tail_semantic_anchor_loss(self, fiber, ids, mask):
        ...
        for b in range(B):
            ...
            loss_general = F.kl_div(log_p0.unsqueeze(0), target_general.unsqueeze(0),
                                    reduction='none').sum(-1).mean()
            losses.append(loss_general)
            if use_rare:
                strict_starters = [t for t in content_tids
                                   if t in cc.strict_content_starter_ids]
                pool = strict_starters if strict_starters else content_tids
                rare_tids = sorted(pool,
                                   key=lambda t: -corpus_idf.get(t, self.c.idf_floor)
                                   )[:self.c.keyword_tail_top_k]
                if rare_tids:
                    target_rare = torch.zeros(V, device=dev)
                    target_rare[rare_tids] = 1.0 / len(rare_tids)
                    slot1_logits = tn[b, 1] @ wn.T / 0.3
                    log_p1 = F.log_softmax(slot1_logits, dim=-1)
                    loss_rare = F.kl_div(log_p1.unsqueeze(0), target_rare.unsqueeze(0),
                                         reduction='none').sum(-1).mean()
                    losses.append(self.c.keyword_tail_weight * loss_rare)
            for s in range(2, n_slots):
                slot_logits = tn[b, s] @ wn.T / 0.3
                log_ps = F.log_softmax(slot_logits, dim=-1)
                losses.append(F.kl_div(log_ps.unsqueeze(0), target_general.unsqueeze(0),
                                       reduction='none').sum(-1).mean())
        if not losses:
