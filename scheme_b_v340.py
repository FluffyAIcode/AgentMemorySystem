...
═══════════════════════════════════════════════════════════════════════════
相对 v3.39 的结构性修复:
[F-1] generate()/prepare_decode_context() 推理路径 update_stats=False,
      memory 在 inference 下 immutable. 修复 4.17.
[F-2] _preserve_min_keep 贯穿所有 retrieval filter,candidate 下界保持到
      rerank 结束. 修复 4.20 / 4.10 / 4.6 / 4.7.
[F-3] fwd-path function suppression: MemLLM.fwd 在 guidance_active 时对
      pure_function tokens 施加 std·scale·dampen 的 structural bias,
      与 shape_step_logits 里的 [E-3] 并存. 修复 4.22 eval-only.
[F-4] WTE 残差 scale 由 target_std·√d 改为 √d_LLM,与 slot_head 的
      post-LN 输出同量级,blend 方向不再被淹没. 修复 4.23.
[F-5] MemoryContextEncoder: 三层 LN + orthogonal init + encode 输出
      per-sample mean-center. 修复 4.24.
[F-6] effective_tail_slots = base + (L_mem-8)//2; 每个 slot s>=1 拿
      memory 的第 s-1 个 rare keyword 作残差,不同 slot 锚定不同内容方向.
      keyword_tail_top_k 从 3 扩到 8. 修复 4.25.
...
class Cfg:
    ...
    fwd_path_bias_dampen: float = 0.25  # [F-7] 0.3 -> 0.25
    ...
    # [F-2] Global min-keep for rerank stability, applied at every filter stage
    ...
    # [F-6] Extended rare keyword count so each of tail slots 1..N-1 can
    #       receive a distinct keyword residual
    ...
    # [F-3] fwd-path function suppression (structural, works at fwd logits
    #       before shape_step_logits; required for probes that hit raw fwd)
    ...
    wte_residual_alpha: float = 0.5   # [F-7] 0.6 -> 0.5
    scale_tail_with_L_mem: bool = True
    # [F-6] New tail scaling rule: base + (L_mem - L_mem_base) // step
    ...
    def effective_tail_slots(self) -> int:
        """[F-6] Scale tail slots linearly with L_mem delta from base."""
...
class MemoryContextEncoder(nn.Module):
    """
    [F-5] Stronger encoder:
      - Per-layer LayerNorm between linears (forces informative intermediate)
      - Orthogonal init (preserves inter-point distances at init, JL-style)
      - encode() mean-centers output before L2-normalize
        => removes the constant-bias drift that pulled all descriptors toward
           a shared direction in v3.39
    """
...
class AMM(nn.Module):
    ...
    def _preserve_min_keep(self, pass_mask, scores, min_keep, diag):
        """
        [F-2] Universal min-keep helper. Ensures at least `min_keep` candidates
        pass, using `scores` as tie-breaker for adding back high-scoring drops.
        Mutates diag.min_keep_enforcements for observability.
        """
...
class MemLLM(nn.Module):
    ...
    def _compute_rare_keyword_wte_residual(self, diag):
        """
        [F-4] Residual scale = sqrt(d_LLM) (matching post-LN slot magnitude).
        [F-6] Slot s in [1, n_slots-1] receives the (s-1)-th rare keyword
              across retrieved memories (weighted by memory weight).
              Different slots -> different content anchors.
        """
        ...
        # [F-4] match LN output magnitude
        target_scale = math.sqrt(self.c.d_LLM)
        residual = torch.zeros(B, n_slots, self.c.d_LLM, device=dev)
        any_nonzero = False
        for b in range(B):
            mw = diag.batch_mem_weights[b]
            for slot_idx in range(1, n_slots):
                kw_rank = slot_idx - 1  # [F-6] each slot -> distinct keyword rank
                ...
                centroid_scaled = centroid * (target_scale / cen_norm)
                residual[b, slot_idx, :] = alpha * centroid_scaled
                any_nonzero = True
    ...
    def fwd(self, ids, mask, prefix=None):
        ...
        # [F-3] fwd-path function suppression (structural)
        ...
        if self.c.use_fwd_path_content_bias and (content_bias is not None
            ...
            if suppression_bias is not None and self.c.use_memory_guided_suppression:
                ...
                mod_last = True
            # [F-3] function suppression via fwd-path
            if need_fs:
                ...
                unit_fn = (logits_std * self.c.content_bias_std_multiplier
                           if self.c.use_adaptive_content_bias_scale else 1.0)
                scale_fn = unit_fn * self.c.fwd_function_suppression_scale * step_scale_fn * dampen
    ...
    def prepare_decode_context(self, ids, mask, update_stats=False):
        """[F-1] Default update_stats=False: memory is immutable during inference."""
    ...
    def shape_step_logits(self, logits_cond, logits_uncond, step,
        ...
        # decode-time functional suppression
    ...
    def generate(self, prompt, mt=50, greedy=False):
        """[F-1] update_stats=False throughout generation => inference is a
        pure function of (model_state, memory_state, prompt, rng_seed)."""
