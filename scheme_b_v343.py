...
嵌入级方案B · v3.43
═══════════════════════════════════════════════════════════════════════════
针对 v3.42 九条 FAIL 的收敛修复:

[I-1] 对称 CFG content_bias(4.10/4.15) — fwd 对 cond/uncond 同步加 cb/sb,
      CFG 差分抵消 bias,总量 = 单次 fwd 加法;dampen 0.25→1.0。
[I-2] 重复惩罚线性化(4.7) — penalty 3.5→2.5, exponent 1.5→1.0, cyclic 3→5。
[I-3] residual 排除 prompt+generated(4.12/4.21)。
[I-4] residual 减 WTE 全局均值(4.23)。
[I-5] hybrid context encoder(4.24)。
[I-6] slot 精确等模长重整(4.25)。
[I-7] 测试作用域内强制确定性执行(4.17)。
"""
...
class MemoryContextEncoder(nn.Module):
    ...
    def encode(self, wte_centroid, hidden_mean=None):
        h_wte = self.proj_wte(wte_centroid.float())
        if self.hybrid and hidden_mean is not None:
            h_hid = self.proj_hid(hidden_mean.float())
            h = h_wte + self.hidden_weight * h_hid
        else:
            h = h_wte
        return F.normalize(h, dim=-1, eps=1e-8)

    def encode_from_tokens(self, content_token_ids, wte, hidden_mean=None):
        ...
        return self.encode(centroid, hidden_mean).detach().contiguous()

    def encode_from_hidden(self, hidden_mean):
        h = self.proj_hid(hidden_mean.float()) if self.hybrid else self.proj_wte(hidden_mean.float())
        return F.normalize(h, dim=-1, eps=1e-8)
...
class MemLLM(nn.Module):
        ...
        if self.bridge.proj.fkv.out_features != 2 * self.c.d_LLM:
            ...
            if self.c.use_memory_context_encoder:
                self.memory_context_encoder = MemoryContextEncoder(
                    ...
                    hidden_weight=self.c.context_hybrid_hidden_weight).to(dev)
    ...
    def _compute_rare_keyword_wte_residual(self, diag, exclude_token_ids: Optional[Set[int]] = None):
        """[I-3]+[I-4] residual exclude + centered."""
    ...
    def fwd(self, ids, mask, prefix=None):
        ...
        # symmetric shaping (cond + uncond both get bias)
    ...
    def shape_step_logits(self, logits_cond, logits_uncond, step,
                          ...
                          mixture_gate=None, memory_logit_bias=None):
        """[I-1] content_bias/suppression_bias 在 fwd 对两侧同步加,CFG 差分抵消。"""
                    ...
                    with torch.no_grad():
                        ...
                        if self.c.context_encoder_source == "wte_strict_starter":
                            ...
                            ctx_desc = self.memory_context_encoder.encode_from_tokens(
                                src_ids, wte_fp32, hidden_mean=hidden_mean_b)
                        else:
                            ctx_desc = self.memory_context_encoder.encode_from_hidden(
                                hidden_mean_b).detach().contiguous()
...
class Trainer:
    ...
    def context_separation_loss(self, texts):
        ...
        hidden_means = self.m._compute_content_semantic_emb(hs_pooled, ids_b, mask_b)
        per_text_strict_ids = []
        for t in texts:
        ...
        for i, ss in enumerate(per_text_strict_ids):
            ...
            centroid = wte.index_select(0, idx).float().mean(0)
            d = self.m.memory_context_encoder.encode(centroid, hidden_mean=hidden_means[i])
            descs.append(d)
        if len(descs) < 2:
            return torch.tensor(0.0, device=dev, requires_grad=True)
